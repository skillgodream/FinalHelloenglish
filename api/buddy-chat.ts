export default async function handler(req: any, res: any) {
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
  res.setHeader("Access-Control-Allow-Headers", "Content-Type");

  if (req.method === "OPTIONS") {
    return res.status(200).end();
  }

  if (req.method !== "POST") {
    return res.status(405).json({
      error: "Method not allowed. Use POST.",
    });
  }

  try {
    const body =
      typeof req.body === "string"
        ? JSON.parse(req.body)
        : req.body || {};

    const {
      history = [],
      learnerMessage,
      message,
      exchangeCount = 1,
      wasAwaitingEnglishRetry: explicitRetry = false,
    } = body;

    const cleanMsg = String(
      learnerMessage || message || ""
    ).trim();

    let wasAwaitingEnglishRetry = Boolean(explicitRetry);

    if (!wasAwaitingEnglishRetry && Array.isArray(history)) {
      const lastBuddyMessage = [...history]
        .reverse()
        .find((item: any) => item.sender === "buddy");

      if (lastBuddyMessage) {
        const text = String(
          lastBuddyMessage.text || ""
        ).toLowerCase();

        wasAwaitingEnglishRetry =
          text.includes("try karo") ||
          text.includes("try saying") ||
          text.includes("main sun raha hoon") ||
          text.includes("ab aap try") ||
          text.includes("aap try karo") ||
          text.includes("in english:") ||
          text.includes("english mein aap keh sakte ho") ||
          text.includes("you can say:") ||
          text.includes("bolo") ||
          text.includes("try");
      }
    }

    const groqKey = process.env.GROQ_API_KEY;

    /*
     * If Groq is not configured, return the same safe
     * response contract used by the existing Buddy API.
     */
    if (!groqKey || !groqKey.trim()) {
      return res.status(200).json({
        understoodMeaning: cleanMsg || "Shared thoughts",
        naturalResponse:
          "I'm listening and understand you completely 😊 Take your time.",
        nextQuestion: "Can you tell me more about that?",
        subtleRecast: cleanMsg || "",
        englishModel: cleanMsg || "",
        awaitingEnglishRetry: wasAwaitingEnglishRetry,
        learnerComfortLanguage: "english",
        newFacts: cleanMsg
          ? [`Learner shared: "${cleanMsg}"`]
          : [],
        topic: "Daily Life",
        conversationDepth:
          typeof exchangeCount === "number"
            ? exchangeCount
            : 1,
        needsClarification: false,
        shouldEnd: false,
        providerUsed: "local_fallback",
        responseTimeMs: 0,
      });
    }

    const systemPrompt = `
You are Buddy, a warm English speaking coach for beginner Indian learners.

Your job is to:
1. Understand the learner's actual meaning, even when English is broken or incomplete.
2. Never criticize the learner.
3. Never say "wrong English" or "bad English".
4. Preserve the learner's actual facts.
5. Give a natural English rephrase.
6. Respond warmly and conversationally.
7. Ask ONE simple follow-up question.
8. If the learner needs to try again in English, gently encourage them.
9. Keep English simple and suitable for a beginner.
10. Do not invent facts.

Return ONLY valid JSON using exactly this structure:

{
  "understoodMeaning": "string",
  "naturalResponse": "string",
  "nextQuestion": "string",
  "subtleRecast": "string",
  "awaitingEnglishRetry": false,
  "learnerComfortLanguage": "english",
  "newFacts": [],
  "topic": "Daily Life",
  "conversationDepth": 1,
  "needsClarification": false,
  "shouldEnd": false
}
`;

    const userPrompt = JSON.stringify({
      conversationHistory: Array.isArray(history)
        ? history.slice(-10)
        : [],
      learnerMessage: cleanMsg,
      exchangeCount,
      wasAwaitingEnglishRetry,
    });

    const startTime = Date.now();

    const response = await fetch(
      "https://api.groq.com/openai/v1/chat/completions",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${groqKey.trim()}`,
        },
        body: JSON.stringify({
          model: "llama-3.1-8b-instant",
          messages: [
            {
              role: "system",
              content: systemPrompt,
            },
            {
              role: "user",
              content: userPrompt,
            },
          ],
          temperature: 0.3,
          max_tokens: 700,
          response_format: {
            type: "json_object",
          },
        }),
      }
    );

    if (!response.ok) {
      throw new Error(
        `Groq API error: ${response.status}`
      );
    }

    const data = await response.json();

    const content =
      data?.choices?.[0]?.message?.content || "";

    let result: any;

    try {
      result = JSON.parse(content);
    } catch {
      const match = content.match(/\{[\s\S]*\}/);

      if (!match) {
        throw new Error(
          "Groq returned invalid JSON"
        );
      }

      result = JSON.parse(match[0]);
    }

    return res.status(200).json({
      understoodMeaning:
        result.understoodMeaning ||
        cleanMsg ||
        "Shared thoughts",

      naturalResponse:
        result.naturalResponse ||
        "That's good. Tell me a little more.",

      nextQuestion:
        result.nextQuestion ||
        "What happened after that?",

      subtleRecast:
        result.subtleRecast || "",

      englishModel:
        result.subtleRecast || "",

      awaitingEnglishRetry:
        Boolean(result.awaitingEnglishRetry),

      learnerComfortLanguage:
        result.learnerComfortLanguage || "english",

      newFacts:
        Array.isArray(result.newFacts)
          ? result.newFacts
          : cleanMsg
            ? [`Learner shared: "${cleanMsg}"`]
            : [],

      topic:
        result.topic || "Daily Life",

      conversationDepth:
        typeof exchangeCount === "number"
          ? exchangeCount
          : 1,

      needsClarification:
        Boolean(result.needsClarification),

      shouldEnd:
        Boolean(result.shouldEnd),

      providerUsed:
        "groq_llama",

      responseTimeMs:
        Date.now() - startTime,
    });

  } catch (err: any) {
    console.error(
      "[Buddy Chat Vercel Error]",
      err?.message || err
    );

    // Never break the learner conversation.
    return res.status(200).json({
      understoodMeaning: "Shared thoughts",

      naturalResponse:
        "I'm listening and understand you completely 😊 Take your time.",

      nextQuestion:
        "Can you tell me more about that?",

      subtleRecast: "",

      englishModel: "",

      awaitingEnglishRetry: false,

      learnerComfortLanguage: "english",

      newFacts: [],

      topic: "Daily Life",

      conversationDepth: 1,

      needsClarification: false,

      shouldEnd: false,

      providerUsed: "local_fallback",

      responseTimeMs: 0,
    });
  }
}